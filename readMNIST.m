...
%%---- My input example:
% [a,b] = readMNIST('train-images-idx3-ubyte', 'train-labels-idx1-ubyte', 300,[0])
...
function [imgs labels] = readMNIST(imgFile, labelFile, readDigits, offset)
    ...
    ShowImages(imgs, labels,readDigits)
end
...
function ShowImages(Dataset,Label,N)
    for i=1:N
        subplot_tight(5,6,i,[0.04,0.04])
        imagesc(Dataset(:,:,i)); colormap(gray(256));
        title(['img ' num2str(i) ', labeled: ' num2str(Label(i))])
    end
end